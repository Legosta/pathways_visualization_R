...
data("kegg.gs")
cancerdata_1 <- cancerdata[,-c(1:3)]
cn_1=colnames(cancerdata_1)
lec_con <- grep('LEC.control',cn_1, ignore.case =TRUE)
lec_vir <- grep('LEC.KSHV',cn_1, ignore.case =TRUE)
cancer_1.kegg.p <- gage(cancerdata_1, gsets = kegg.gs,
                        ref = lec_con, samp = lec_vir)
cancer_1.kegg.2d.p <- gage(cancerdata_1, gsets = kegg.gs,
